          ...
          return (
            <ListItem key={layer.id} className="item-layer">
              ...
              {isChecked[layer.id] &&
                ...
                <img hspace="7" src={layer.icon} alt= "icono" 
                 height="20" width="20" />}
              
              {isChecked[layer.id] && layer.link &&
              <IconButton color = "primary" href = {layer.link} target = "_blank"> <CloudDownloadIcon /> </IconButton>
              }
            </ListItem>
          );
        })
  ...
  return (
    <>
      <div className="contenedor-layers">
        ...
        <List dense={true} className="lista-layers">
          {handlePrivacy()}
