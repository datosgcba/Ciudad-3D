...
class MapaInteractivoGL {
      ...
      const builder = (this._layerBuilders[layerId] || !layer[layerId].builder)?layerId:layer[layerId].builder;
          try {
              if (this._layerBuilders[builder]) {
                  ...
                  self.map.on('mouseenter',  sourceId, function () {
                     self.map.getCanvas().style.cursor = 'pointer';
                  });
                    
                  // Change it back to a pointer when it leaves.
                  self.map.on('mouseleave',  sourceId, function () {
      ...
      if(icon.iconUrl && icon.shadowUrl){
        ...
        const layerIcon = {
            ...
            onAdd: function() {              
              ...
              self._request_image(icon.shadowUrl, function(img){                              
                context.drawImage(img, 0, 0, size, size);
                self._request_image(icon.iconUrl,function(img){
                  context.drawImage(img, 0, 0, size, size);    
    ...
    addMarker(latlng, visible = true, draggable = false, goTo, activate = true, clickable = true, options = {}) {           
      ...
      if(clickable){
        ...
        // Change the cursor to a pointer when the mouse is over the places layer.
        self.map.on('mouseenter', marker, function () {
          self.map.getCanvas().style.cursor = 'pointer';
        });
          
        // Change it back to a pointer when it leaves.
        self.map.on('mouseleave', marker, function () {
          self.map.getCanvas().style.cursor = '';
        });
